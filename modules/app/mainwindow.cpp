...
void MainWindow::slot_importJson()
{
    ...
    qDebug() << item;
    QJsonValue sub_obj1 = item["tracker"];
    QJsonValue sub_obj2 = item["version"];
    QJsonValue sub_obj3 = item["date"];
    qDebug() << sub_obj1.toString() << endl << sub_obj2.toString() << endl << sub_obj3.toString();
    ...
    foreach (const QJsonValue & value, attributes) {
        ...
        // CATEGORY
        QJsonValue category = obj.value("category").toString();
        qDebug() << category.toString();

        // LABEL
        QJsonArray label = obj.value("label").toArray();
        int size = label.size();
        for(int i = 0; i < size; i++){
            QString answer = label.at(i).toString();
            //FrameBasedData::setLabel(value2.toStdString().c_str());
            qDebug() << answer;
        }
    }

    // FRAME
    QJsonArray frame = json_Obj["frames"].toArray();
    foreach (const QJsonValue & value, frame) {
        QJsonObject obj = value.toObject();

        // ID
        QJsonValue id = obj.value("id").toString();
        ...
        int size = name.size();
        for(int i = 0; i < size; i++){
            QString answer = name.at(i).toString();
            qDebug() << answer;
        }
...
//            }
        ...
        for(int i = 0; i < size3; i++){
            QString answer = comments.at(i).toString();
            qDebug() << answer;
        }*/

        // Boxes
        /*QJsonArray boxes = obj.value("boxes").toArray();
        int size4 = boxes.size();
        for(int i = 0; i < size4; i++){
            QString value2 = boxes.at(i).toString();
            qDebug() << value2;
        }*/
    }
